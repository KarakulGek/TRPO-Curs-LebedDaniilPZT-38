...
begin
  ...
  for var j:integer := 0 to Inventory.ItemListBox.Items.Count - 1 do begin
   //Если в инвентаре присутствует меч или амулет, то делает соответствующие картинки видимыми
   if Inventory.ItemListBox.Items[j] = 'Меч' then SwordImage.Visible := True
   else if Inventory.ItemListBox.Items[j] = 'Амулет' then AmuletImage.Visible := True;
  end;
